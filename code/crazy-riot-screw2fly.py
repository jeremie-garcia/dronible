...
# this example uses rotation on the Z axis to make the drone takeoff above a certain threshold.
# then the rotation is used to compute the vertical speed (in both directions)
    ...
    try:
        ...
        def process_gyro(_x, _y, _z):
            ...
            if drone.motion_commander is not None:
                if state == 0:
                    if not drone.motion_commander._is_flying and abs(_z) > 0.5:
                        state = 1
                        drone.take_off()
